...
package com.twocities.pomodoro.widget.swipelistview;
...
//		}
	...
	private void setBackView(View backView) {
		...
		if(backView != null) {
			mBackLeftView = backView.findViewById(R.id.todo_left_view);
			mBackRightView = backView.findViewById(R.id.todo_right_view);
		}
...
//		});
	...
	private void generateDismissAnimate(final View view, final boolean swap,
		...
		ViewPropertyAnimator.animate(view).translationX(moveTo).alpha(alpha)
	...
	private void generateRevealAnimate(final View view, final boolean swap,
		...
		ViewPropertyAnimator.animate(view).translationX(moveTo)
				.setDuration(animationTime)
				.setListener(new AnimatorListenerAdapter() {
					@Override
					public void onAnimationEnd(Animator animation) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity
				...
				if (opened.get(downPosition) && openedRight.get(downPosition)
					...
					// no matter how fast the user swipe,
					// the deltaX must longer than half of viewWith  
...
//			}
	...
	public void move(float deltaX) {
		if (deltaX > 0) {
			...
			mBackLeftView.setVisibility(View.VISIBLE);
			mBackRightView.setVisibility(View.GONE);
		} else {
			backView.setBackgroundResource(R.color.swipe_back_item_background_del);
			mBackLeftView.setVisibility(View.GONE);
			mBackRightView.setVisibility(View.VISIBLE);
		}
